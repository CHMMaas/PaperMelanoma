...
table(sel.data$Status, sel.data$dead)
...
for (outcome in c("Rec.5", "MSM.5.refit")){
  for (arm in c("Positive", "Negative")){
    ...
    S.5.subset <- S.5[last.imputed.data.set$SNstatus==arm,]
    
    cindex.stratified <- rep(0, m)
    cindex.se.stratified <- rep(0, m)
    for (i in 1:m){
      subset.imputed.data <- as.data.frame(mice::complete(imputed.data,i)[last.imputed.data.set$SNstatus==arm,])
      rc <- Hmisc::rcorr.cens(-predict(f.mi.stratified,newdata=subset.imputed.data, type="lp"), S.5.subset)
...
print(data.to.be.imputed[patient.nr, c("SNstatus", "Age.SN", "Ulceration", "Loc_CAT", "Breslow", "Rdamcrit")])
