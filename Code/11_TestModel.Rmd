...
# MSM.cal.fact for only last imputed data set
lp.5<-predict(f.mi.BS.Rec.5,
              newdata=mice::complete(imputed.training.data,5), 
              type="lp")
f.mi.5 <- rms::cph(S.MSM.5.training~lp.5,y=TRUE,x=TRUE) 
MSM.cal.fact.5 <- stats::coef(f.mi.5)

sum(h.Rec$density*fun.event(lp=lp.points.Rec.5, h0=h0.Rec.5))
...
mean(fun.event(lp=lp.Rec, h0=h0.Rec.5))
```
...
mean(fun.event(lp=lp.MSM, h0=h0.MSM.5))
```
