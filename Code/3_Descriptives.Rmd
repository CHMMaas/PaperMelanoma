...
survminer::ggsurvplot(survival::survfit(S.Rec.training ~ Center, data=last.imputed.training.data.set), 
                      legend.title="",
...
survminer::ggsurvplot(survival::survfit(S.MSM.training ~ Center, data=last.imputed.training.data.set), 
                      legend.title="",
                      conf.int=TRUE, risk.table=TRUE)+
  ggplot2::guides(colour=guide_legend(nrow=2))
grDevices::dev.off()
...
survminer::ggsurvplot(survival::survfit(S.Rec.validation ~ 1, data=last.imputed.validation.data.set), 
                      legend.title="",
                      legend.labs="Center=Australia",
                      conf.int=TRUE, risk.table=TRUE)
grDevices::dev.off()
png(file=paste0("Z:/Project Melanoom/PaperMelanoma/Results/KM.MSM.validation.png"), width=16, height=16, units="cm", res=300)
survminer::ggsurvplot(survival::survfit(S.MSM.validation ~ 1, data=last.imputed.validation.data.set), 
                      legend.title="",
                      legend.labs="Center=Australia",
                      conf.int=TRUE, risk.table=TRUE)
grDevices::dev.off()
...
median.time <- function(data=NA, type=NA){
  ...
  tab.time <- c("Total Development", paste(sprintf("%.2f", median.time.total$quantiles.survival[3, "quantile"]),
                   paste0("[", sprintf("%.2f", median.time.total$quantiles.survival[4, "quantile"]), "; ",
                          sprintf("%.2f", median.time.total$quantiles.survival[2, "quantile"]), "]")))
  ...
  for (center in c("Amsterdam", "Berlin", "Rotterdam", "Warsaw", "Australia")){
    ...
    lower <- med.table[4,"quantile"]
    upper <- med.table[2,"quantile"]
