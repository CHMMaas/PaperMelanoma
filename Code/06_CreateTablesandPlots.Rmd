...
for (model in list("Rec.5", "MSM.5.refit")){
  ...
  for (type in c("full", "BS")){
    ...
    out <- PredictionTools::val.surv.mi(p=p, y=S, g=5, main="All centers", CI.metrics=TRUE,
                                        time=NULL, lim=c(0,1), dist=TRUE)
    grDevices::dev.off()
    
    final.full.table <- eval(parse(text=paste0("final.", type, ".table.", model)))
    final.full.table["Harrell's C-index", "HR"] <- sprintf("%.2f", out$cindex-optimism)
    final.full.table["Harrell's C-index", "CI"] <- paste0("[", sprintf("%.2f", out$cindex.lower-optimism),
                                                   "; ", sprintf("%.2f", out$cindex.upper-optimism), "]")
    final.full.table["Uno's C-index", "HR"] <- sprintf("%.2f", out$uno.C-optimism)
...
for (type in c("full", "BS")){
  ...
  for (i in 1:m){
    ...
    unoC.MSM[i] <- survC1::Est.Cval(mydata=cbind(S.MSM.5.training[, 1], 
                                              S.MSM.5.training[, 2], 
                                              lp.i), 
                                 tau=5, nofit=TRUE)$Dhat
  }
  ...
  final.full.table["Harrell's C-index MSM", "HR"] <- sprintf("%.2f", 
                                                cindex.mi.MSM.cal$est-optimism)
  final.full.table["Harrell's C-index MSM", "CI"] <- paste0("[", 
    sprintf("%.2f", cindex.mi.MSM.cal$est+qnorm(.025)*cindex.mi.MSM.cal$se-optimism),
    "; ", sprintf("%.2f", cindex.mi.MSM.cal$est+qnorm(.975)*cindex.mi.MSM.cal$se-optimism), "]")
  final.full.table["Uno's C-index MSM", "HR"] <- sprintf("%.2f", mean(unoC.MSM)-optimism)
...
row.names(final.full.table.Rec.5) <- c("Positive SN status", "Male",
                                    ...
                                    rownames(final.full.table.Rec.5)[(nrow(final.full.table.Rec.5)-3):nrow(final.full.table.Rec.5)])
...
row.names(final.full.table.MSM.5.refit) <- c("Positive SN status", "Male",
                                          ...
                                          "Subcap", "Combined", "Parenchymal",
                                          "Multifocal", "Extensive",
                                          row.names(final.full.table.MSM.5.refit)[(nrow(final.full.table.MSM.5.refit)-1):nrow(final.full.table.MSM.5.refit)])
...
row.names(final.BS.table.Rec.5) <- c("SN status",
                                     ...
                                     "Breslow (positive)",
                                     "SN tumour burden", 
                                   row.names(final.BS.table.Rec.5)[(nrow(final.BS.table.Rec.5)-3):nrow(final.BS.table.Rec.5)])
...
row.names(final.BS.table.MSM.5.refit) <- c("SN status",
                                     ...
                                     "Breslow (positive)",
                                     "SN tumour burden",  
                                   row.names(final.BS.table.MSM.5.refit)[(nrow(final.BS.table.MSM.5.refit)-1):nrow(final.BS.table.MSM.5.refit)])
