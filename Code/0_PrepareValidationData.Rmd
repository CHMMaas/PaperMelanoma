...
# remove space after Negative # Bespreken met Robert
levels(original.validation.data$SNstatus)[levels(original.validation.data$SNstatus)=="Negative "] <- "Negative"

# all patients may be included
sel.val.data <- original.validation.data
```
...
```{r, eval=TRUE}
sel.val.data$Histology[sel.val.data$Histology=="LLM"] <- "other" # Bespreken met Robert, is LLM and LMM the same?
```

#### Define multiple fields
```{r, eval=TRUE}
sel.val.data$multiple.fields <- as.numeric(!is.na(sel.val.data$Tot_nr_SNs)&sel.val.data$Tot_nr_SNs>1) # Bespreken met ask Robert
```
...
Impute recurrence time for 45 patients with missing Rec.time but Recurrence=="recurred"
```{r, eval=TRUE}
cat('Number of patients dead with recurrence without Date_1st_Rec:', sum(is.na(sel.val.data$Date_1st_Rec)&sel.val.data$Status=="Dead, melanoma       "), '\n')
cat('Number of patients alive with recurrence without Date_1st_Rec:', sum(is.na(sel.val.data$Date_1st_Rec)&sel.val.data$Status=="Alive with Melanoma  "))
```

Set recurrence indicator
```{r, eval=TRUE}
Rec.date <- as.numeric(!is.na(sel.val.data$Date_1st_Rec)) # n=1294
Rec.date <- factor(Rec.date, labels=c("no recurrence", "recurred"))
table(sel.val.data$Status, Rec.date) # Bespreken met Robert

cat("Number of alive patients with Date_1st_Rec, but status NSR:", sum(!is.na(sel.val.data$Date_1st_Rec)&sel.val.data$Status=="Alive NSR            "), "\n")
cat("Number of alive patients with Date_1st_Rec, but status unknown:", sum(!is.na(sel.val.data$Date_1st_Rec)&sel.val.data$Status=="Alive, status unknown"), "\n")
cat("Number of dead patients with Date_1st_Rec, but status unknown:", sum(!is.na(sel.val.data$Date_1st_Rec)&sel.val.data$Status=="Dead, cause unknown  "), "\n")
cat("Number of dead patients with Date_1st_Rec, but status not melanoma:", sum(!is.na(sel.val.data$Date_1st_Rec)&sel.val.data$Status=="Dead, not melanoma   "), "\n")
