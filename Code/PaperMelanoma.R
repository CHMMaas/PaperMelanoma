...
# Exclude based on duplicates, urogenital melanoma, etc.
sel.data <- original.data[original.data$EXCLUSIE!="Exclusion", selected.vars] # 2 = Exclusion
...
save(data.to.be.imputed,file='./Data/descr.data.Rdata', compress=TRUE)
...
# save(imputed.data,file='./Data/imputed.data.RData', compress=TRUE)
...
write.table(as.data.frame(tab.FU.time),file='./Results/med.FU.table.txt',
            row.names=FALSE)
tab.FU.time
...
Rec.table <- rbind(Rec.table, c("Positive", sum(sel.data$Recurrence=="recurred"&sel.data$SNstatus=="Positive"),
                                sprintf("%.1f", sum(sel.data$Recurrence=="recurred"&sel.data$SNstatus=="Positive")/sum(sel.data$SNstatus=="Positive")*100)))
Rec.table <- rbind(Rec.table, c("Negative", sum(sel.data$Recurrence=="recurred"&sel.data$SNstatus=="Negative"),
                                sprintf("%.1f", sum(sel.data$Recurrence=="recurred"&sel.data$SNstatus=="Negative")/sum(sel.data$SNstatus=="Negative")*100)))
utils::write.table(as.data.frame(Rec.table),file='./Results/Rec.table.txt',
                   row.names=FALSE)
Rec.table

#####
##### MELANOMA SPECIFIC MORTALITY AT FIVE YEARS
#####
...
utils::write.table(as.data.frame(MSM.table),file='./Results/MSM.table.txt',
                   row.names=FALSE)
MSM.table
...
utils::write.table(rbind(c("All patients:", sprintf('%.2f', cindex.mi.AJCC.7$est),
            ...
            file='./Results/AJCC.7.Cindex.txt', row.names=FALSE, col.names=FALSE)
...
utils::write.table(rbind(c("All patients:", sprintf('%.2f', cindex.mi.AJCC.8$est),
            ...
            file='./Results/AJCC.8.Cindex.txt', row.names=FALSE, col.names=FALSE)
...
utils::write.table(table.AJCC.7.8,
            file='./Results/table.AJCC.7.and.8.txt', row.names=FALSE, col.names=FALSE)
...
form.BS.Rec <- S.Rec ~
  ...
  #10 Dewar +
  #7 SNstatus:Sex +
  #6 SNstatus:log(Age.SN) +
  #9 SNstatus:Ulceration +
  #4 SNstatus:Loc_CAT +
  #1 SNstatus:Histology +
  SNstatus:I(log(Breslow)-c.Breslow)
  #2 SNstatus:multiple.fields
...
###
### Melanoma specific mortality, RECALIBRATE
...
for (model in list('Rec', 'MSM.refit')){
  if (model == "Rec"){
    # plot predict
    png(file=paste0("./Results/Figures/plot.predict.Rec.png"))
    plot.predict <- plot(Predict(f.mi.full.Rec))
    show(plot.predict)
    grDevices::dev.off()

    # plot predict Breslow
    png(file=paste0("./Results/Figures/plot.predict.Breslow.Rec.png"))
  ...
  for (type in c("full", "BS")){
    ...
    out <- val.surv.mi(p,S,g=5,main="All centers",
                       file_path=paste0("./Results/Figures/all.centers.", model, '.', type, ".png"),
    ...
    for (j in 1:4){
      ...
      for (select.value in c("Positive", "Negative", "All")){
        ...
        out <- val.surv.mi(p, S, g=5, main=paste("Center", Centers.anonimized[j]),
                           file_path=paste0("./Results/Figures/", model, '.', select.value, '.', type, '.', Centers[j], ".png"),
                           save_plots=TRUE,time=NULL,lim=c(0,1),dist=TRUE)
      }
    }
  }
...
for (select.value in c("All", "Positive", "Negative")){
  for (y in c("Rec", "MSM")){
    png(file=paste0("./Results/", y, ".", select.value, ".png"), width=16, height=16, units="cm", res=300)
    par(mar=rep(0, 4))
    layout(matrix(1:4, ncol=2, byrow=TRUE))
    Centers.val <- rep(1:4, 3)
    for (i in 1:4){
      plot(NA, xlim=0:1, ylim=0:1, xaxt="n", yaxt="n", bty="n")
      img <- readPNG(paste0("./Results/Figures/", y, ".", select.value, ".BS.", Centers[Centers.val[i]], ".png"))
...
y.lim <- c(0, 100)
y.lim.hist <- c(0, 25)
png(file=paste0("./Results/Risk.distribution.png"))
par(mar = c(5,5,2,5))
