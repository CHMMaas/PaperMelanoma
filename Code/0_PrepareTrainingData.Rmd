...
# load data file
original.train.data <- foreign::read.spss(file="Z:/Project Melanoom/PaperMelanoma/Data/training/Dataset Predictiemodel.sav",
...
very close.
```{r, eval=TRUE}
sel.train.data <- original.train.data[original.train.data$EXCLUSIE!="Exclusion", selected.vars]
cat("Number of patients excluded:", nrow(original.train.data)-nrow(sel.train.data), "\n")
```
...
```{r, eval=TRUE}
sel.train.data$Rec.time[sel.train.data$Recurrence=="no recurrence"|is.na(sel.train.data$SNdate)]<-sel.train.data$FU.time[sel.train.data$Recurrence=="no recurrence"|is.na(sel.train.data$SNdate)]
table(sel.train.data$Status, sel.train.data$Recurrence)
```
...
sel.train.data$multiple.fields <- factor(sel.train.data$multiple.fields, labels=c("Other", "Multiple SN fields"))
sel.train.data$MSM <- factor(sel.train.data$MSM, labels=c("Other", "Death by melanoma"))
```

#### Describe years
```{r, eval=TRUE}
dates <- as.POSIXct(sel.train.data$SNdate, origin="1582-10-14")
Years <- as.numeric(format(dates, format="%Y"))
utils::write.table(cbind(table(Years), table(Years, sel.train.data$Recurrence)[,2], 
                         table(Years, sel.train.data$MSM)[, 2]),
                   file=paste0("Z:/Project Melanoom/PaperMelanoma/Results/Time.Rec.MSM.txt"),
                   row.names=TRUE, sep=",")
hist(Years, main="")
```
...
```{r, eval=TRUE}
training.data.to.be.imputed$logRec.time <- log(sel.train.data$Rec.time+1/365.25)
training.data.to.be.imputed$logFU.time <- log(sel.train.data$FU.time+1/365.25)
missings.table(training.data.to.be.imputed)
save(list=c("dd.training", "training.data.to.be.imputed"),file="Z:/Project Melanoom/PaperMelanoma/Data/training/training.data.Rdata", compress=TRUE)
```
